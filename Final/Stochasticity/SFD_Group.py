...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"time_step": 2, "shock_seed": 1},
)
def economic_factor():
    return (
        np.random.uniform(
            1 - 1 / np.sqrt(time_step()), 1 + 1 / np.sqrt(time_step()), size=()
        )
        * 0.53333
    )
...
@component.add(name="shock seed", comp_type="Constant", comp_subtype="Normal")
def shock_seed():
    return 1
